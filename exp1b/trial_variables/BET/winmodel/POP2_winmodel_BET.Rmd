...
Originally had loss streak length in but realised it doesn't make sense - sometimnes there are 2 wins in a row - and a preceding win is quan. different from a preceding loss. Also this is what outcome of trial was beore the cirrent outcome. Doesn't really make sense.
...
```{r}
modeldata=load_data('BET', 'win','POP2')
```
...
model1<-glm(Next.bet.binary~Participant.f
            ...
            -1
            ,data=modeldata,family=binomial())
print.mod.summary(model1,subset(modeldata,Participant!=19))
```
...
robmodel1<-glmrob(Next.bet.binary~Participant.f
            ...
            + Final.balance
            -1,
               family=binomial, data=subset(modeldata,Participant!=19),method="Mqle")
...
filename = "POP2_robmodel_win_BET"
title = "Experiment 1b: Predicting bet size after a win"
pcol="p value"
...
## Does game mode change anything?
Error in running study - machne was not consistentlydsiplaying cash or credit. Luckily evenly balanced between the groups.
