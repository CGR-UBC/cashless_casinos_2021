...
```{r}
modeldata=load_data('ISI', 'win','POP2')
```
...
write.table(tail(robustmodel,5),"POP2_robust_model.csv",sep=",")
...
filename = "POP2_robmodel_win_ISI"
title = "Experiment 1b: Predicting spin iniitation latency after a win"
pcol="p value"
...
robust_diagnostics(logmodel1,robmodel1)
```
...
write.table(tail(robustmodelrev,5),"POP2_robust_model_gp_rev.csv",sep=",")

data<-(tail(robustmodelrev,5))
data<-data[,c(1,2,6,7,5)]

data<-rename_cols_for_tables(data)  
data<-rename_rows_for_tables(data)  

filename = "POP2_robmodel_win_ISI_gp_rev"
title = "Experiment 1a: Predicting spin iniitation latency after a win: group reversed"
pcol="p value"
...
```{r}
robmodel3<-lmrob(logISI~Participant.f
                + sqrtTrial.no  
             + Binary.bet.change
             + logResult
                  + logResult:group
                  + logResult:Game_mode
             + logResult:group:Game_mode
             + Final.balance
             -1
            ,data=modeldata, control = lmrob.control(setting="KS2014") )
summary(robmodel3)
```

```{r}

robmodel3b<-lmrob(logISI~Participant.f
            + sqrtTrial.no  
             + Binary.bet.change
             + logResult
                  + logResult:group
                  + logResult:Game_mode_rev
             + logResult:group:Game_mode_rev
             + Final.balance
             -1
            ,data=modeldata,control=lmrob.control(fast.s.large.n = Inf,setting="KS2014"))
summary(robmodel3b)
...
```{r}
robustmodel<-tidy.glmrob(robmodel3,conf.int=TRUE)
write.table(tail(robustmodel,7),"robust_model_GAME_MODE.csv",sep=",")
robustmodel<-tidy.glmrob(robmodel3b,conf.int=TRUE)
